<?php
	...
	class Account extends Db
	{
		...
		public function __construct(string $method_name)
		{
			...
			$this->oldpassword  	= $_POST['oldpassword'] ?? false;
		...
		/**
		 * Update user data (Profile page)
		 *
		 * @method userdataChangeOperation
		 */

		public function userdataChangeOperation() : void
		{
			$this->query('UPDATE users SET username = ? WHERE id = ?', [$this->username, $_SESSION['user']['id']]);

			$_SESSION['user']['username'] = $this->username;

			die(\json_encode('User data has been successfully changed'));
		}
		 ...
		 *
		 * @method passwordChangeOperation
		 */

		public function passwordChangeOperation() : void
		{
			if ($this->password !== $this->password_repeat)
				die(\json_encode(['error' => 'Passwords do not match!']));
